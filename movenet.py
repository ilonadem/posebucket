...
def main():
    ...
    while cap.isOpened():
        ...
        if ret == True:
            ...
            img = np.expand_dims(img, axis=0)
            print("RESIZED IMAGE SHAPE: ", np.shape(img))
            ...
            n += 1
            y, x, _ = frame.shape
            poses.append(np.squeeze(np.multiply(keypoints_with_scores, np.array([y, x, 1]))))
            if n%100 ==0:
                pickle.dump(poses, open(f"./data/data_{n}", "wb"))
                poses = []
