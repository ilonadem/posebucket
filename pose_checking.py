...
def main():
    ...
    def render_overlay(engine, output, src_size, inference_box):
        nonlocal n, sum_process_time, poses, sum_inference_time, fps_counter
        ...
        for pose in outputs:
            ...
            for label, keypoint in pose.keypoints.items():
                ...
                poses.append(sv_pose_str)
            n+= 1

            if n%1000 == 0:
                print(f"pickling! at n={n}")
                dt_string = datetime.datetime.now().strftime("%d_%m_%H_%M_%S")
                pickle.dump(poses, open(f"./data/data_{dt_string}.pickle", "wb"))
                poses = []
